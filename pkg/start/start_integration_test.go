...
func TestIntegrationCVO_initializeAndUpgrade(t *testing.T) {
	...
	worker := cvo.NewSyncWorker(retriever, cvo.NewResourceBuilder(cfg, cfg, nil), 5*time.Second, wait.Backoff{Steps: 3}, "", record.NewFakeRecorder(100), func() (string, error) {
		return "", nil
	})
...
func TestIntegrationCVO_initializeAndHandleError(t *testing.T) {
	...
	worker := cvo.NewSyncWorker(retriever, cvo.NewResourceBuilder(cfg, cfg, nil), 5*time.Second, wait.Backoff{Duration: time.Second, Factor: 1.2}, "", record.NewFakeRecorder(100), func() (string, error) {
		return "", nil
	})
	controllers.CVO.SetSyncWorkerForTesting(worker)
...
func TestIntegrationCVO_gracefulStepDown(t *testing.T) {
	...
	worker := cvo.NewSyncWorker(&mapPayloadRetriever{}, cvo.NewResourceBuilder(cfg, cfg, nil), 5*time.Second, wait.Backoff{Steps: 3}, "", record.NewFakeRecorder(100), func() (string, error) {
		return "", nil
	})
	controllers.CVO.SetSyncWorkerForTesting(worker)
